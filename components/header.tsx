...
export default function Header() {
  return (
    <header className='inset-x-0 top-0 z-50 py-6'>
      <nav className='container flex max-w-3xl items-center justify-between pb-3 pt-3'>
        <div className='w-8 content-center'>
          <Link href='/' className=''>
            <svg
              width='20'
              height='20'
          ...
          {Object.entries(navItems).map(([path, { name }]) => {
            return (
              <Link
                key={path}
                href={path}
                className='relative m-1 flex px-2 py-1 align-middle text-sm transition-all hover:text-neutral-800 dark:hover:text-neutral-200'
              >
                {name}
              </Link>
            )
        ...
        <div className='h-8 w-8 content-center'>
          <ThemeToggle />
        </div>
      </nav>
    </header>
